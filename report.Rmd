...
X <- cbind("Intercept" = 1 , amr[,c(pois_vars, "ln_population")]) %>%
  as.matrix(.) %>%
  t()
#X["ln_population",] <- 0 # set population to zero to calculate rate
...
pal <- colorNumeric("OrRd", domain = admin$`Zero-corrected Predicted AMR Events`, na.color = "#e9e9f0")
...
lf <- leaflet() %>%
  ...
  addPolygons(data = admin, 
              ...
              fill = TRUE, fillColor = ~pal(`Zero-corrected Predicted AMR Events`), fillOpacity = 0.9,
              label = ~paste0(name, ": ", `Zero-corrected Predicted AMR Events`)) %>%
  addLegend(data = admin, pal = pal, values = ~`Zero-corrected Predicted AMR Events`, position = "bottomright") 

lf

htmlwidgets::saveWidget(lf, h("plots/map_predictions.html"))
