...
out_zi <- map_df(c("ln_pubs_sum", "ln_gdp_dollars", "ln_population"), function(var){
  ...
  sampy <- map_dfc(1:10000, function(i){
    ...
    seqy_lm <- b_zi_Intercept + 
      b_zi_ln_pubs_sum * x_ln_pubs_sum +
      b_zi_ln_gdp_dollars * x_ln_gdp_dollars +
      b_zi_ln_population * x_ln_population
...
p2 <- me_combined %>% 
  get_me_dat(.) %>%
  filter(var %in% c("ln_pubs_sum", "ln_gdp_dollars", "ln_population")) %>%
...
out_pois <- map_df(c("ln_livestock_consumption_kg_per_pcu", "ln_livestock_pcu",
                     "ln_migrant_pop_perc",  "ab_export_perc", "health_expend_perc",
                     "human_consumption_ddd", "ln_gdp_dollars"), function(var){
                       ...
                       sampy <- map_dfc(1:10000, function(i){
                         ...
                         seqy <- exp(b_Intercept + 
                                       ...
                                       b_ln_gdp_dollars * x_ln_gdp_dollars + 
                                       b_ln_livestock_consumption_kg_per_pcu * x_ln_livestock_consumption_kg_per_pcu + 
                                       b_ln_livestock_pcu * x_ln_livestock_pcu + 
                                       b_ln_migrant_pop_perc * x_ln_migrant_pop_perc +
                                      x_ln_population
                         )
                         seqy
                       })
...
p2 <- me_combined %>% 
  get_me_dat(.) %>%
  filter(var %in% c("ln_livestock_consumption_kg_per_pcu", "ln_livestock_pcu",
                    "ln_migrant_pop_perc",  "ab_export_perc", "health_expend_perc", "human_consumption_ddd",      
                    "ln_gdp_dollars")) %>%
  ggplot(., aes(x = value, y = estimate__)) + 
  geom_line(color = "green") +
  geom_ribbon(aes(ymin=lower__, ymax=upper__), alpha = 0.2, fill = "green") +
