...
beta_means <- apply(betas, 2, mean) %>%
  enframe(name = "variable", value = "beta") %>%
  mutate(variable = str_remove(variable, "b_"))
intercept <- beta_means$beta[beta_means$variable=="Intercept"]

amr_pois <- amr %>%
  select(country, continent, pois_vars, ln_population) %>%
  gather(key = "variable", value = "value", -country, -continent) %>%
  left_join(beta_means) %>%
  mutate(prod = value * beta)

amr_pois_pred <- amr_pois %>%
  group_by(country) %>%
  mutate(total_pois_pred = exp(sum(prod) + intercept)) %>%
  ungroup() %>%
  left_join(amr_predict)

# each country parts
amr_pois_pred %>%
  mutate(country= fct_reorder(country, total_pois_pred)) %>%
  filter(variable != "ln_population") %>%
  filter(n_amr_events >0) %>%
  ggplot(data = ., aes(x = country, y = prod, fill = variable)) + 
...
p_pred <- amr_pois_pred %>%
  filter(n_amr_events == 0) %>%
  mutate(country= fct_reorder(country, total_pois_pred)) %>%
  select(country, continent, total_pois_pred, mod_predict ) %>%
  gather(key = "prediction", value = "value", -country, -continent) %>%
  ggplot(., aes(x = country, y = value, color = prediction)) +
  geom_point() +
...
ggsave(plot = p_pred, filename = h("plots/zero_predictions.png"), width = 12, height = 22)

# as interval - run with distribtions of betas
# adapt above but x is instead country values
