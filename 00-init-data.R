...
# SM.POP.TOTL = International migrant stock, total (number of people born in a country other than that in which they live. It also includes refugees)
...
wbd %<>%
  ...
  rename(wb_population = SP.POP.TOTL,
         wb_migrant_pop_perc = SM.POP.TOTL,
...
consumption <- readxl::read_xlsx(h("Supplementary Table 1 Spreadsheet Data[2].xlsx")) %>%
  ...
  rename(iso3c = `International Organization for Standardization (ISO)*`, 
         im_ab_consumption =  `CCDEP Usage Per Capita (Units of usage are on average 45% of DDD measured by ECDC)`) %>%
  na.omit() %>%
  filter(iso3c != "na")
